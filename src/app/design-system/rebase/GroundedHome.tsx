...
function GroundedCard({ firm }: { firm: PropFirm }) {
    ...
    <article className={`${styles.card} ${compared ? styles.cardSelected : ''}`}>
      <header className={styles.cardHeader}>
        <Link href={`/prop-firms/${firm.slug}`} className={styles.identity}>
          ...
          <span className={styles.identityCopy}>
            <span><BadgeCheck aria-hidden="true" /> Demo profile</span>
      ...
      <div className={styles.tags}>
        ...
        {firm.rewardTags?.slice(0, 3).map((tag) => (
          <span key={tag} className={tag === 'Points' ? styles.pointsTag : tag === 'Airdrop' ? styles.airdropTag : styles.tokenTag}>{tag}</span>
        ))}
      </div>
...
export function GroundedHome({ firms }: { firms: PropFirm[] }) {
    ...
    <main className={styles.lab}>
      <div className={styles.labShell}>
        <nav className={styles.labNav} aria-label="Design lab navigation">
          ...
          <Link className={styles.labNavActive} href="/design-system/rebase">Rebase</Link>
          <Link href="/design-system/sage">Sage field</Link>
        </nav>

        <div className={styles.prototype}>
          <header className={styles.siteHeader}>
            <a className={styles.wordmark} href="#top" aria-label="PropHub rebase prototype home">
              <span>PH</span> PropHub
            </a>
            <nav aria-label="Prototype navigation">
              <a href="#firms">Prop firms</a>
              <a href="#firms">Compare</a>
              <a href="#firms">Rewards</a>
          ...
          <section className={styles.hero} id="top">
            <div className={styles.chartLayer}><HeroChartBackdrop /></div>
            <div className={styles.heroCopy}>
              <span className={styles.eyebrow}><BadgeCheck aria-hidden="true" /> Straight answers before you buy</span>
              <h1>Find the right on-chain prop firm.</h1>
              <strong>Compare the rules, proof and rewards before you pay.</strong>
              <p>See the price, drawdown rules and reward programs in one place. Open the full research only when you need it.</p>
              <div className={styles.heroActions}>
                <a href="#firms">Browse prop firms <ArrowRight aria-hidden="true" /></a>
                <a href="#about">How comparisons work</a>
              </div>
              <div className={styles.heroStats}>
                <span><strong>10</strong> firms in the demo</span>
                <span><strong>3</strong> key costs up front</span>
                <span><strong>Points</strong> and airdrops tracked</span>
                <span><strong>Clear</strong> data labels</span>
              </div>
            </div>
          </section>
          ...
          <section className={styles.directoryPreview} id="firms">
            <header className={styles.sectionHeading}>
              <div><span>Start here</span><h2>Three profiles worth opening first.</h2></div>
              <div><p>The important trade-offs stay visible. The full rulebook is one click away.</p><Link href="/prop-firms">View all firms <ArrowRight aria-hidden="true" /></Link></div>
            </header>
            ...
            <div className={styles.cardGrid}>{firms.map((firm) => <GroundedCard key={firm.id} firm={firm} />)}</div>

            <div className={styles.browseRow}>
              <div><span>Not sure where to begin?</span><strong>Compare two firms side by side.</strong></div>
              <Link href="/compare">Open comparison <GitCompareArrows aria-hidden="true" /></Link>
            </div>
          </section>

          <section className={styles.about} id="about">
            <div><span>Simple by default</span><h2>Useful for a first look. Detailed when it matters.</h2></div>
            <div className={styles.aboutSteps}>
              <article><strong>1</strong><h3>Scan the basics</h3><p>Price, profit split, funding and drawdown stay on every card.</p></article>
              <article><strong>2</strong><h3>Check the labels</h3><p>Demo, reported and verified data never look the same.</p></article>
              <article><strong>3</strong><h3>Open the proof</h3><p>Use the detailed profile when a firm makes your shortlist.</p></article>
            </div>
          </section>

          <footer className={styles.prototypeFooter}><span>Grounded Dark / rebase candidate</span><p>Old mood, current structure, violet as the primary action.</p></footer>
        </div>
      </div>
    </main>
  );
