...
export function FirmCard({ firm, onCompareToggle, isCompared }: FirmCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute -top-14 -left-14 w-44 h-44 bg-[#52b788]/[0.05] rounded-full blur-3xl pointer-events-none group-hover:bg-[#52b788]/[0.08] transition-colors" />
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-between space-y-5">
        ...
        <div className="space-y-5">
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-4 min-w-0">
              ...
              <div className="relative shrink-0">
                <img
                  src={firm.logo}
                  alt={firm.name}
                  className="h-16 w-16 rounded-2xl object-cover border border-zinc-800/80 shadow-sm"
                />
              </div>
              ...
              <div className="min-w-0 space-y-1">
                <div className="flex items-center gap-1.5 min-w-0">
                  <span className="text-base font-bold text-white tracking-tight group-hover:text-zinc-200 transition-colors truncate block">
                    {firm.name}
                  </span>
            ...
            {/* Compare Toggle Button (with event propagation stop) */}
            {onCompareToggle && (
              <button
                type="button"
                onClick={(e) => {
                ...
                className={`p-2 rounded-xl border text-xs transition-colors shrink-0 z-20 ${
                  isCompared
          ...
          {/* DOMINANT METRICS WITH LARGER NUMBERS AND CLOSER ICONS */}
          <div className="grid grid-cols-3 gap-2 py-2 text-left items-start">
            ...
            <div className="flex flex-col items-start text-left">
              ...
              <span className="text-zinc-500 block text-[10px] uppercase font-medium tracking-wider mt-1 text-left">
                From
              </span>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-1.5 text-[11px] font-medium">
            ...
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-[#1c1c20] border border-zinc-800/80 text-zinc-300">
              <GitCommit className="h-3 w-3 text-zinc-400 shrink-0 stroke-[1.5]" />
              <span>{firm.evaluationSteps.join(' / ')}</span>
            </span>
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-[#1c1c20] border border-zinc-800/80 text-zinc-300">
              <Monitor className="h-3 w-3 text-zinc-400 shrink-0 stroke-[1.5]" />
      ...
      <div className="relative z-10 pt-4 border-t border-zinc-800/40 flex items-center justify-between gap-3 font-satoshi">
        ...
        <span className="flex-1 flex items-center justify-center gap-1.5 py-2.5 px-4 rounded-xl bg-zinc-200/80 group-hover:bg-white text-zinc-950 font-bold text-xs transition-colors shadow-sm">
